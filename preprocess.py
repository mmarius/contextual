    ...
    def make_hdf5_file(self, sentences: List[str], out_fn: str) -> None:
        ...
        with h5py.File(out_fn, 'w') as fout:
            for sentence in Tqdm.tqdm(sentences):
                ...
                fout.create_dataset(str(sentence_index), embeddings.shape,
                                    dtype='float32', data=embeddings)
                sentence_index += 1
...
class BertBaseCased(Vectorizer):
    ...
    def vectorize(self, sentence: str) -> numpy.ndarray:
        ...
        with torch.no_grad():
            embeddings, _, input_embeddings = self.model(
...
class GPT2(Vectorizer):
    ...
    def vectorize(self, sentence: str) -> numpy.ndarray:
        ...
        token_ids_tentative = self.tokenizer.convert_tokens_to_ids(
            tokens_tentative)
        ...
        for i, tid in enumerate(token_ids_tentative):
            ...
            if tid != 0:
                ...
                special_char_tid = self.tokenizer.convert_tokens_to_ids(
                    'Ġ' + tokens_tentative[i])
                # otherwise, break up the word using the given GPT2 tokenizer
                subtoken_tids = self.tokenizer.convert_tokens_to_ids(
                    self.tokenizer.tokenize(tokens_tentative[i]))
...
def index_sentence(data_fn: str, index_fn: str, tokenize: Callable[[str], List[str]], min_count=5) -> List[str]:
    ...
    with open(data_fn) as csvfile:
        ...
        for line in csvreader:
            ...
            index_tokens(tokenize(line['Sent1']),
                         sentence_index, word2sent_indexer)
            index_tokens(tokenize(line['Sent2']),
                         sentence_index + 1, word2sent_indexer)
    ...
    infrequent_words = list(filter(lambda w: len(
        word2sent_indexer[w]) < min_count, word2sent_indexer.keys()))
...
if __name__ == "__main__":
    ...
    EMBEDDINGS_PATH = "/logfiles/contextual-embeddings/bert"
    ...
    bert = BertBaseCased()
    sentences = index_sentence(INPUT_FILE, os.path.join(
        EMBEDDINGS_PATH, 'word2sent.json'), bert.tokenizer.tokenize)
